...
double drand() { return (((double)(rand() % RAND_MAX)) / (((double)RAND_MAX) - 1.0)); }

// Wrap function

int wrap(int n, int min, int max) { return ((((n - min) % (max - min)) + max - min) % (max - min)) + min; }
...
void switch_bad_default(const char *switch_type, const char *filename, int lineno) {
  cerr << filename << ":" << lineno << " " << switch_type << " was some value we didn't expect" << endl;
}
...
void animate_smoke(linkedlist<smoketype> &smoke) {
  ...
  while (smoke.next()) {
    ...
    smoke().time++;
    if (smoke().time == int(28 / GAME_SPEED))
      smoke.kill();
  }
}
...
void animate_flames(linkedlist<firetype> &flame, linkedlist<smoketype> &smoke) {
  ...
  while (flame.next()) {
    ...
    if (flame().time == int(280 / GAME_SPEED))
      flame.kill();
  }
}
...
void rotate_radars(linkedlist<radartype> &radar) {
  ...
  while (radar.next()) {
    ...
    if (radar().rotate == 0)
      radar().image = wrap(radar().image + 1, 238, 246);
  }
}
...
void clone_planes(linkedlist<plane> &p, linkedlist<planeclone> &dp, int mission, int &winner) {
  ...
  while (p.next()) {
    ...
    if ((mission == 2) && (dp().buildingwin == 0) && (p().land == 0) && (!p().drak)) {
      winner = p().side;
      p().targetx = -10;
    }
  }
...
void plane_collisions(gamedata &g) {
  ...
  while (g.p.next()) {
    ...
    while (g.dp.next()) {
      if (g.p().id > g.dp().id) {
        if ((g.p().state < 3) && (g.dp().state < 3) && ((g.p().state < 2) || (g.dp().state < 2))) {
          if (g.images[g.p().image + g.p().d].collide(int(g.p().x), int(g.p().y), g.images[g.dp().image + g.dp().d],
                                                      int(g.dp().x), int(g.dp().y))) {
...
void move_shots(linkedlist<shottype> &shot, shape &ground, shape &shotimage, shape &drakmsimage, drakmstype &drakms) {
  ...
  while (shot.next()) {
    ...
    if ((ground.collide(0, 0, shotimage, (int)shot().x, (int)shot().y)) || (shot().x < -16.0) ||
        (shot().x > GAME_WIDTH) || (shot().y < -16.0) || (shot().y > GAME_HEIGHT) || (shot().time == 0) ||
        ((drakms.exist == 1) &&
         (drakmsimage.collide((int)drakms.x, (int)drakms.y, shotimage, (int)shot().x, (int)shot().y)))) {
      shot.kill();
    }
  }
}
...
void gunshoot(guntype &gun, linkedlist<shottype> &shot, sampleio &sound, double xmove[17], double ymove[17]) {
...
// Fire guns
void fire_guns(linkedlist<guntype> &gun, linkedlist<plane> &p, sampleio &sound, building b[MAP_W * 2],
               linkedlist<shottype> &shot, double xmove[17], double ymove[17]) {
  ...
  while (gun.next()) {
    ...
    if (gun().target == 0) {
      ...
      p.reset();
      while (p.next()) {
        if ((gun().side != p().side) && (p().state < 2) && (p().land == 2) && (gun().y > p().y) && (!p().hide)) {
      ...
      while (p.next()) {
        if (p().id == gun().target) {
          ...
          if ((abs(int(dx)) > 200) || (p().state > 1) || (p().hide) || (p().land != 2)) {
            gun().target = 0;
          }
        }
      }
