...
bool fall_collision(gamedata &g, falltype &fall) {
  ...
  for (int x = px; x <= px + 1; x++) {
    ...
    case 5: // buildings and trees
      if (g.images[g.gamemap.b[x].image].collide(g.gamemap.b[x].x, g.gamemap.b[x].y, g.images[fall.image], (int)fall.x,
                                                 (int)fall.y)) {
  ...
  if (g.drakms.exist == 1) {
    ...
    while (g.drakgun.next()) {
      if (g.images[g.drakgun().image[g.drakgun().d]].collide((int)g.drakms.x + g.drakgun().x,
                                                             (int)g.drakms.y + g.drakgun().y, g.images[fall.image],
                                                             (int)fall.x, (int)fall.y)) {
