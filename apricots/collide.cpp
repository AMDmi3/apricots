...
void detect_collisions(gamedata &g) {
  ...
  for (int x = px; x <= px + 1; x++) {
    ...
    default:
      switch_bad_default("building.type", __FILE__, __LINE__);
      break;
    }
  }
  ...
  g.shot.reset();
  while (g.shot.next()) {
    if (g.images[114].collide((int)g.shot().x, (int)g.shot().y, g.images[g.p().image + g.p().d], (int)g.p().x,
                              (int)g.p().y)) {
      if (g.p().state < 2) {
        g.p().score -= 50;
  ...
  if (g.drakms.exist == 1) {
    ...
    while (g.drakgun.next()) {
      if (g.images[g.drakgun().image[g.drakgun().d]].collide(
              (int)g.drakms.x + g.drakgun().x, (int)g.drakms.y + g.drakgun().y, g.images[g.p().image + g.p().d],
              (int)g.p().x, (int)g.p().y)) {
        if (g.p().state < 2)
          g.p().score -= 50;
        g.p().score += 50;
  ...
  if (g.p().state < 2) {
    ...
    g.flame.reset();
    while (g.flame.next()) {
      if (g.images[94].collide(g.flame().x, g.flame().y, g.images[g.p().image + g.p().d], (int)g.p().x,
                               (int)g.p().y)) {
        g.p().score -= 10;
        g.p().state = 2;
        g.p().land = plane::LandingState::FLYING;
    ...
    g.fall.reset();
    while (g.fall.next()) {
      if (g.images[g.fall().image].collide((int)g.fall().x, (int)g.fall().y, g.images[g.p().image + g.p().d],
                                           (int)g.p().x, (int)g.p().y)) {
        g.p().score -= 50;
